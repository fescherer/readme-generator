...
import { TItemText } from '@/@types/item'
...
export function ListText({ fields, remove }: ListProps) {
  return (
    <Accordion.Root collapsible type="single" className="mt-4 flex flex-col gap-2 overflow-hidden rounded">
      {fields.map(field => (
        <div key={field.keyId} className="flex w-full">
          <Accordion.Item value={field.keyId} className="w-full overflow-hidden rounded border border-zinc-300 ">
            <Accordion.Header>
          ...
          <button className="rounded bg-slate-100 p-2 transition-all hover:text-form-hightlight" type="button" onClick={() => remove(fields.findIndex(item => item.keyId === field.keyId))}>
            <LucideX size={16} />
          </button>
        </div>
      ))}
